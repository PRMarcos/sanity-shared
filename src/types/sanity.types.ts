...
// Query: *[_type == "ourSmedsPage"][0]{    title,  "bannerImage": bannerImage.asset->url,  description,  conclusion,}
...
// Query: *[_type == "eventsPage"][0]{    _id,    title,    "bannerImage": bannerImage.asset->url,  }
...
// Query: *[_type == "event" && _id == $id && isActive == true][0] {  title,  about,  shortDescription,  "address": address->{    title,    street,    city,    state,    zip  },  "banner": banner.asset->url,  "bannerMobile": bannerMobile.asset->url,  "eventColor": eventColor.hex,  organizer {    description,    "phone": phone->{      number,      name    },    "email": email->{      address,      name    }  },  "schedule": schedule[]{    date,    sessions[]{      title,      description,      starTime,      endTime    }  },  "background": background.asset->url,  "speakers": speakers[]->{    name,    birthDate,    title,    titleAbbreviation,    bio,    "image": photo.asset->url  },  teaser}
...
// Query: *[_type == "sermonSummary" && _id == $id && isActive == true][0] { _id,  "allTags": *[_type == "sermonTag"] {_id,title},  title,  date,  "slug": slug.current,  "background": background.asset->url,  "speaker": speaker->{    name,    birthDate,    title,    titleAbbreviation,    biography,    "image": photo.asset->url  },  "tags": tags[]->{    _id,    title  },  "videoLink": videoUrl,  content}
...
// Query: *[_type == "sermonSummary" && isActive == true] {   _id,      title,   date,  tags[]->{_id,title},   "slug": slug.current,   "background": background.asset->url,   speaker->{ name, titleAbbreviation, "photo":photo.asset->url},    }
...
// Query: *[_type == "smed"] | order(_createdAt desc) {      _id,      title,      "banner": banner.asset->url,      "bannerHorizontal": bannerHorizontal.asset->url,      smedDescription,      smedButton    }
...
// Query: *[_type == "homePage"][0].eventList[]->{    _id,      title,      shortDescription,      "address":address->title,      schedule[] {        date,      },      "background": background.asset->url    }
...
// Query: *[_type == "sermonSummary" && isActive == true ] | order(date desc)[0...5] {      _id,      title,      date,      "slug": slug.current,      "background": background.asset->url,      speaker->{ name, titleAbbreviation, "photo":photo.asset->url}          }
...
// Query: *[_type == "sermonSummaryPage"][0]{    _id,    title,    "bannerImage": bannerImage.asset->url,  }
